...
from transformers import RobertaModel, WhisperConfig, WhisperModel
from transformers.models.whisper import modeling_whisper as whisper_model

from whisper import CustomWhisperEncoderLayer
from modules.crossmodal_transformer import CrossModalTransformerEncoder
from .transformer_facial_mmt import AdditiveAttention, MELDTransEncoder
...
class MultimodalFusionModel(nn.Module):
    def __init__(
        ...
        modalities: List[str],
        feature_type: str,
        use_cross_modal: bool,
        use_raw_text: bool,
        use_cam_loss: bool,
        use_raw_audio: bool,
        whisper_use_adapters: bool,
        cam_type: str,
    ):
        super().__init__()
        ...
        if self.feature_type == "pooled_features":
            ...
            if "A" in modalities and use_raw_audio:
                ...
                self.whisper_model.embed_positions = nn.Embedding(
                    max_positions, self.whisper_model.config.d_model
                )
                self.whisper_model.embed_positions.weight.data[:] = original_weights[
                    :max_positions
                ]
                ...
                if self.whisper_use_adapters:
                    ...
                    num_total_param = (
                        sum(p.numel() for p in self.whisper_model.parameters()) / 1e6
                    )
            ...
            self.text_transformer = (
                MELDTransEncoder(
                    config=config, layer_num=1, get_max_lens=38, hidden_size=hidden_dim
                )
                if "T" in modalities
                else None
            )
            self.audio_transformer = (
                MELDTransEncoder(
                    config=config, layer_num=1, get_max_lens=60, hidden_size=hidden_dim
                )
                if "A" in modalities
                else None
            )
            self.video_transformer = (
                MELDTransEncoder(
                    config=config, layer_num=1, get_max_lens=40, hidden_size=hidden_dim
                )
                if "V" in modalities
                else None
            )
            ...
            if self.cam_type == "T_to_CAM":
                ...
                self.audio_seq_classifier = (
                    nn.Linear(hidden_dim, num_classes) if "A" in modalities else None
                )
                self.video_seq_classifier = (
                    nn.Linear(hidden_dim, num_classes) if "V" in modalities else None
                )
                ...
                self.text_seq_classifier = (
                    nn.Linear(hidden_dim, num_classes) if "T" in modalities else None
                )
            ...
            if self.use_cross_modal:
                ...
                self.crossmodal_ta_v = (
                    ...
                    if "T" in modalities and "V" in modalities
                    else None
                )
                self.crossmodal_av = (
                    CrossModalTransformerEncoder(
                    ...
                    if "A" in modalities and "V" in modalities
                    else None
                )
    ...
    def forward(self, inputs: Dict[str, torch.Tensor], labels: torch.Tensor = None):
        ...
        if "T" in self.modalities and self.use_raw_text:
            ...
            word_level_features = torch.zeros(batch_size, 38, hidden_size).to(
            ...
            if target_start_pos is not None and target_end_pos is not None:
                for i in range(batch_size):
                    ...
                    if curr_utt_len > 38:
                        curr_utt_len = 38
                    if curr_utt_len > 0:
                        word_level_features[i, :curr_utt_len] = text_feat[
                            i, start : start + curr_utt_len
                        ]
            ...
            text_mask = (text_seq.sum(dim=-1) != 0).float()
            text_extended_mask = (
                1.0 - text_mask.unsqueeze(1).unsqueeze(2)
            ) * -10000.0
            ...
            text_feat = text_feat.transpose(1, 2)
            text_mask = (
                F.interpolate(text_mask.unsqueeze(1), size=self.target_seq_len, mode="linear")
                .squeeze(1)
            )

            if self.use_cam_loss and self.cam_type == "T_to_CAM":
                text_cam, text_logits, text_loss, text_time_weights = self.text_cam(
                    text_feat, labels=labels
                )
            ...
            text_seq = self.text_proj(inputs["T"])
            text_feat = self.text_transformer(text_seq, attention_mask=None)
        ...
        if "A" in self.modalities and self.audio_proj is not None and self.use_raw_audio:
            ...
            if self.use_cam_loss and self.cam_type == "AV_to_CAM":
                audio_cam, audio_logits, audio_loss, audio_time_weights = self.audio_cam(
                    audio_feat, labels=labels
                )
        ...
        if "V" in self.modalities and self.video_proj is not None:
            ...
            if self.use_cam_loss and self.cam_type == "AV_to_CAM":
                video_cam, video_logits, video_loss, video_time_weights = self.video_cam(
                    video_feat, labels=labels
                )
            elif self.use_cam_loss and self.cam_type == "T_to_CAM":
        ...
        if self.use_cam_loss:
            ...
            if self.cam_type == "T_to_CAM":
                ...
                if "V" in self.modalities:
                    ...
                    weighted_kl_tv = (kl_div_tv * text_time_weights.unsqueeze(-1)).mean()
                    cam_loss += 0.5 * weighted_kl_tv

        if self.use_cross_modal:
            if "T" in self.modalities and "A" in self.modalities and self.crossmodal_ta is not None:
                text_feat = text_feat.transpose(0, 1)
                audio_feat = audio_feat.transpose(0, 1)
                text_cross_audio = self.crossmodal_ta(text_feat, audio_feat, audio_feat)
                audio_cross_text = self.crossmodal_ta(audio_feat, text_feat, text_feat)
                ta_cross_feat = text_cross_audio
            else:
                if "T" in self.modalities:
                    ta_cross_feat = text_feat.transpose(0, 1)
                if "A" in self.modalities:
                    ta_cross_feat = audio_feat.transpose(0, 1)

            if "V" in self.modalities and self.crossmodal_ta_v is not None:
                video_feat = video_feat.transpose(0, 1)
                video_cross_ta = self.crossmodal_ta_v(video_feat, ta_cross_feat, ta_cross_feat)
                ta_cross_video = self.crossmodal_ta_v(ta_cross_feat, video_feat, video_feat)
                final_feat = ta_cross_video
            elif "V" in self.modalities and self.crossmodal_av is not None:
                video_feat = video_feat.transpose(0, 1)
                video_cross_ta = self.crossmodal_av(video_feat, ta_cross_feat, ta_cross_feat)
                ta_cross_video = self.crossmodal_av(ta_cross_feat, video_feat, video_feat)
                final_feat = ta_cross_video
            else:
        ...
        if self.use_cam_loss and self.cam_type == "Tcam_to_CAM":
            return logits, cam_loss, text_loss
        if self.use_cam_loss and self.cam_type == "AVcam_to_CAM":
            return logits, cam_loss, audio_loss, video_loss
        return logits
